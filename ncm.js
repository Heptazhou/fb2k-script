			...
			for (var k in song) {
				...
				for (var a_k in song[k].artists) {
					...
					if (p0 > out[0]) {
						...
						c = a_k
						out[0] = p0
					} else if (!artist && p0 == out[0] && p1 > out[1]) {
						b = k
						c = a_k
						out[1] = p1
					}
						...
						switch (lrc_order[key]) {
							case "new_merge":
								if (issetlrc && issettran) {
									...
									newLyric.Source = get_my_name() + sourcename + " (并排)"
							...
							case "origin":
								if (issetlrc) {
									...
									newLyric.Title = res_name + outstr1
									newLyric.Artist = res_artist + outstr2
									newLyric.Source = get_my_name() + sourcename + " (原词)"
									callback.AddLyric(newLyric)
								}
								break
							case "tran":
								if (issettran) {
									...
									newLyric.Title = res_name + outstr1
									newLyric.Artist = res_artist + outstr2
									newLyric.Source = get_my_name() + sourcename + " (翻译)"
									callback.AddLyric(newLyric)
								}
								break
							case "newtype":
								if (issetlrc && issettran) {
									...
									newLyric.Title = res_name + outstr1
									newLyric.Artist = res_artist + outstr2
									newLyric.Source = get_my_name() + sourcename + " (并列)"
									callback.AddLyric(newLyric)
								}
								break
							case "old_merge":
...
function lrc_merge(olrc, tlrc) {
	...
	for (var ii = 5, set = 0, counter; ii < 10; ii++) {
		//玄学取set...
